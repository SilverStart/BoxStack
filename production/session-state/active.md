...
Rebuild WebGL and retest on the phone. Confirm the visible build marker shows `B002`, Korean UI text renders, then check whether `friction = 2.4`, `linearDamping = 0.6`, and `angularDamping = 0.8` reduce lower-box sliding without making the stack feel glued.
