...
Task: Replace prototype OnGUI UI layer
<!-- /STATUS -->
...
- UI exploration: Four HUD mockup variants were generated in `design/ui/hud-variants/`; variant 01 Toss Minimal is now applied to the prototype HUD for playtest. Result popup is currently an `OnGUI` prototype overlay with restart by button release or `R`, while undo is now a separate HUD button below the top bar instead of a fail-popup continuation. WebGL Korean text fallback now uses `Assets/Resources/Prototype/Fonts/NotoSansKR-VF.ttf` applied to prototype GUI styles. A tiny `B014` prototype build marker is shown below the top-right HUD area so mobile WebGL rebuild/caching can be checked quickly.
...
- Playtest verdict: Basic game system and box-stacking feel are acceptable for prototype continuation. B013 phone testing confirmed that the separate HUD undo button fits later-stage recovery better than fail-popup continuation, and the change was committed as `12692f8 되돌리기 버튼 방식으로 변경`.
...
- `BoxStackPrototypeConfig` now defines the stage/tuning boundary, and a checked-in `Resources` config asset exists for inspector tuning. PC WebGL smoke confirmed the rebuilt player loads and behaves with B014 values; phone browser spot testing is still useful for real device safe-area and touch feel.
