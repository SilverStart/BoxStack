...
Task: Verify B014 prototype config extraction
<!-- /STATUS -->
...
- Stage plan: A 20-stage fixed-clear difficulty plan is recorded at `design/quick-specs/20-stage-difficulty-plan-2026-05-04.md` and now wired through `BoxStackPrototypeConfig` fallback data. Stage navigation is available during Editor Play with previous/next keyboard controls within the unlocked range, and tapping/clicking the HUD stage label opens a simple 20-stage select overlay where locked stages are disabled. After playtesting, stages 10+ feel significantly harder, so the prototype now uses a stage-limited HUD undo button instead of lowering the difficulty table. Main mode should use fixed box-count clears; infinite stacking is deferred as a later challenge-mode experiment.
...
- `BoxStackPrototypeConfig` now defines the stage/tuning boundary, but there is not yet a checked-in config asset; fallback defaults preserve behavior until an asset is needed.
