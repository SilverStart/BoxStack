...
Task: Delivery Arcade HUD direction planning
<!-- /STATUS -->
...
- UI exploration: Four HUD mockup variants were generated in `design/ui/hud-variants/`; variant 01 Toss Minimal is now applied through a runtime UI Toolkit layer for playtest. On 2026-05-13, the current UI was judged too app-like for motivation, so a research pass and A/B/C mockup sheet were added at `design/ui/boxstack-game-ui-research-2026-05-13.md` and `design/ui/mockups/boxstack-ui-directions-2026-05-13.png`. Direction A, Delivery Arcade, is now the approved next HUD direction; implementation guidance lives in `design/ui/boxstack-delivery-arcade-hud-spec-2026-05-13.md`. Result popup restart, stage select, and the HUD undo button are currently handled by `BoxStackPrototypeUi` instead of the previous runtime `OnGUI` drawing path. WebGL Korean text fallback still uses `Assets/Resources/Prototype/Fonts/NotoSansKR-VF.ttf`, now assigned to UI Toolkit text elements. A tiny `B015` prototype build marker is shown below the top-right HUD area so mobile WebGL rebuild/caching can be checked quickly.
