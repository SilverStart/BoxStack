...
- 날짜: 2026-05-16
- 빌드 마커: B071
- 기준 커밋: `6d330fa B071 phone WebGL 점검 문서 정리`
- 플랫폼: AIT WebGL Dev Server, phone browser
- 세션 유형: milestone browser/device spot check
- 해당 검증은 사용자의 수동 phone 테스트로 진행한다.
- 결과 기록일: 2026-05-17
- 최종 판정: 실패. phone WebGL 실기기에서 상단 중앙 박스 인디케이터, 클리어 결과 팝업, 스테이지 선택 팝업이 화면 밖으로 잘려 보인다.
...
|---|---:|---|---|---|---|---|---|
| 1 | 4 | 로드는 됨 | 실패 | 실패 | 문제 보고 없음 | 문제 보고 없음 | 상단 중앙 인디케이터가 화면 밖으로 잘림 |
| 5 | 6 | 재점검 필요 | 실패 | 실패 | 재점검 필요 | 재점검 필요 | B073 수정 후 재확인 |
| 10 | 8 | 재점검 필요 | 실패 | 실패 | 재점검 필요 | 재점검 필요 | B073 수정 후 재확인 |
| 15 | 10 | 재점검 필요 | 실패 | 실패 | 재점검 필요 | 재점검 필요 | B073 수정 후 재확인 |
| 20 | 12 | 재점검 필요 | 실패 | 실패 | 재점검 필요 | 재점검 필요 | B073 수정 후 재확인 |

## 판정

- phone WebGL 로드/캐시: 로드는 됨, 캐시는 B073에서 재확인
- phone safe-area HUD: 실패
- 한국어 폰트: 문제 보고 없음, B073에서 재확인
- touch 입력: 팝업이 잘려 보이므로 B073에서 재확인
- WebGL 성능: 문제 보고 없음, B073에서 재확인
- 즉시 수정 필요 여부: 필요
- 다음 행동: B073에서 UI Toolkit safe-area 좌표 변환을 수정하고 phone WebGL로 재점검한다.
