...
상태:
- B110에서 첫 구현을 완료했고, 사용자 Editor Play 확인에서 수용됐다.

목표:
- `ResolveDrop` 안에 있던 안정된 드롭 박스의 placed stack 편입 책임을 `BoxStackPrototype` 밖으로 분리한다.
- 드롭 안정 대기, 실패 판정, 바닥 다중 접촉 확인, 배치음, 클리어 검증/다음 박스 전환은 기존 메인 런타임 흐름에 남겨 동작을 바꾸지 않는다.

구현 결과:
- `BoxStackDroppedBoxSettlement`가 placed tint 적용, `SettledGravityScale` 적용, `_placedBoxes` 목록 추가를 담당한다.
- `BoxStackPrototype.ResolveDrop`은 정착 편입 세부를 새 클래스에 위임하고, 성공/실패/다음 흐름 조정만 유지한다.
- 일반 배치, 빗나간 드롭 실패, 목표 개수 후 클리어 검증, 배치음과 색상 흐름은 바꾸지 않았다.
...
상태:
- B113에서 첫 구현을 완료했고, 자동 검증에서 통과했다.

목표:
- `BoxStackPrototypeUiStateFactory` 안에 있던 결과 팝업 제목, 본문, 버튼 라벨 조립을 별도 문구 경계로 분리한다.
- UI 표시 상태, 스테이지 버튼 상태, 팝업 표시 여부 조립은 기존 UI state factory에 남겨 동작을 바꾸지 않는다.

구현 결과:
- `BoxStackResultPresentation`이 성공/실패 결과 제목, `탑이 안정됐어요`, `최고 기록 갱신`, `탑이 무너졌어요`, `탑 밖으로 박스가 떨어졌어요`, 결과 버튼 라벨을 조립한다.
- `BoxStackPrototypeUiStateFactory`는 결과 표시 상태일 때 `BoxStackResultPresentation`이 반환한 값을 `BoxStackPrototypeUi.UiState`로 전달한다.
- 결과 팝업 문구와 버튼 라벨은 B091/B107/B112 기준을 유지한다.
...
상태:
- B114에서 첫 구현을 완료했고, 자동 검증에서 통과했다.

목표:
- `ResolveDrop` 안에 있던 드롭 시작 시간, stable window 시작 시간, 최소 resolve 시간 경과 여부, 안정 유지 시간 완료 여부 판단을 별도 경계로 분리한다.
- 실제 물리 안정 판정, 실패 판정, 정착 처리, 배치음, 클리어 검증/다음 박스 전환은 기존 흐름에 남겨 동작을 바꾸지 않는다.

구현 결과:
- `BoxStackDropStabilityWait`이 resolve 시작 시각과 stable window 시작 시각을 보관한다.
- `BoxStackPrototype.ResolveDrop`은 최소 resolve 시간이 지났고 `BoxStackDropPhysics.StackMotionIsStable`이 true일 때만 stable window 완료 여부를 묻는다.
- 스택이 불안정하면 기존처럼 stable window를 초기화한다.
