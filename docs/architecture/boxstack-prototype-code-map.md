...
- B039 추상 블록 모드에서는 기존 택배 PNG 대신 코드 생성형 테두리 있는 단색 블록/팔레트 기반 바닥/팔레트 기반 배경을 선택.
...
- B039는 배송/택배 시각 테마를 걷어내고 Stack-like 2D 디자인 가이드에 맞춰 스테이지별 유사 색 계열 팔레트, 테두리 있는 단색 컬러 블록, 코드 생성형 뒷배경 그라데이션, 반투명 미니멀 HUD를 사용합니다. B039 변경은 B038의 테두리 있는 단색 블록과 위로 갈수록 진해지는 색상 흐름을 유지하면서, 이동 중 박스와 착지 후 박스가 같은 틴트를 사용하게 해 착지 순간 색이 더 진해지는 변화를 제거합니다.
